...
<html lang="en">
    ...
    <body>
		...
		if ($getmajor_id = $mysqli -> query("SELECT id FROM programs WHERE name = '" . $_GET['programq'] . "'")) {
		...
		<script type = "text/javascript">
					function emptyFields(){
						if(document.getElementById("numArtsCourses").value == ""){
							...
							location.href = "https://roata.myweb.cs.uwindsor.ca/Self-Student%20Counselling/page2.php?message=Please+fill+in+all+fields";
							return false;
						}
        ...
        <div class="container">
          <div class="row">            
			...
			if($getcourses = $mysqli -> query("SELECT code, name FROM courses WHERE id IN (SELECT course_id FROM course_program WHERE program_id = " . $major_id . ") ORDER BY code ASC")){
				$courseCodes = [];
				echo "
				<div class=\"col\">
					<table class=\"table\">
				...
				while($row = $getcourses -> fetch_row()){
					...
					$prep_term = "";
					if($getterm = $mysqli -> query("SELECT name FROM semesters WHERE id IN (SELECT semester_id FROM course_semester WHERE course_id IN (SELECT id FROM courses WHERE code = '". $row[0] ."' ORDER BY code DESC))")){
				...
				echo "
						...
						<form autocomplete=\"off\" action=\"page3.php\" method=\"post\">
							...
							. $_SESSION["term"] . "\">
						<button type=\"button\" class=\"btn btn-primary mb-3\" onclick = \"location.href='https://roata.myweb.cs.uwindsor.ca/Self-Student%20Counselling/page.php';\">Back</button>
          ...
          <!--<button type="submit" class="btn btn-primary" href="page.php">Back</button><!--dont use?-->
          <!-- <a class="btn btn-primary" href="page.php" role="button">Back</a> <!--how to submit form data with link?-->
         <!-- <button type="submit" class="btn btn-primary" formaction="page3.php">Continue</button><!--dont use?-->
          <!--<a class="btn btn-primary" href="page3.php" role="button">Continue</a> <!--how to submit form data with link?-->
