...
class Program4
{
    ...
    public function __construct(int $program)
    {
        ...
        $this->compsci_courses_3000 = 2;
        $this->business_courses = 4;
    }
    ...
    public function requirement_business(array &$user_courses)
    {
        ...
        foreach ($user_courses as $course) {
            ...
            if (($lettercode[0] == "ACCT" || $lettercode[0] == "MGMT" || $lettercode[0] == "MKTG" || $lettercode[0] == "STEN" || $lettercode[0] == "MSCI") && !in_array($course, $this->major_courses) && !in_array($course, $exceptions)) { //check if a non-major course is a business course
                $user_key = array_search($course, $user_courses);
                unset($user_courses[$user_key]);
                $user_courses = array_values($user_courses);
                $viable++;
    ...
    public function buildCourseTable($year, $current_num_courses_added, &$courses_this_term, &$remaining_business_courses, &$remaining_cs_3000, &$remaining_arts_courses, &$remaining_soc_courses, &$remaining_artssoc_courses, &$remaining_electives)
    {
